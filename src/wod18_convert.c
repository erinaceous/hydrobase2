...
*   
*   USAGE: wod18_convert filelist -S<shipfile> -W<wmo_squares_file> -T<instr_type> 
*        [-B<badfile>] [-D<dirname>] [-E<extent>] [-h] [-O<outfile>] 
*        [-R/[b/min_depth/max_depth]/[w]
----------------------------------------------------------------------------------
...
*
* Feb 2018 changes by Jim Bacon
*   - Linting
*   - Add check_depth_flag function. Protects against inverted casts.
*   - Fix bug where originator depth flag overwrote WOD depth flag.
*   - Reject stations with missing metadata such as lat and long.
*   - Reject stations with seabed depth outside known range.
*   - Prevent division by zero when calculating decimation of properties.
*   - Reject stations in non-oceanic WMO squares.
*
*/
#include <stdio.h>
#include <stdlib.h>
...
/************************************************************
 ...
 Variables for oceanic WMO square codes from external files

 num_wmo_squares  - Number of WMO square codes (lines) read from file
 wmo_squares - WMO squares table from the lists/WMO_squares.list file.
...
/************************************************************
 ...
 Variables for setting constraints on imported data 

 b_flag  - Set to 1 to apply seafloor depth limits
 minpdr - If b_flag == 1 then only import records with depth >= minpdr.
 maxpdr - If b_flag == 1 then only import records with depth <= maxpdr.
 w_flag - Set to 1 to limit records to squares in the wmo_squares_file.

*************************************************************/

int b_flag = 0, w_flag = 0;
int minpdr = 0, maxpdr = 11000;
...
int main(int argc, char **argv)
{
  ...
  /* Buffer to store WMO square */
  char wmo_square[5];

  char *dir, *extent, *st, *s;
  FILE *shipfile, *wmo_squares_file;
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    if (argv[i][0] == '-')
    {
      ...
      case 'W': /* Open wmo_squaes_file  */
        wmo_squares_file = fopen(&argv[i][2], "r");
        if (wmo_squares_file == NULL)
        {
          fprintf(stderr, "\nError opening wmo_squares file: %s\n", &argv[i][2]);
          exit(1);
        }
        break;
      ...
      case 'R':
        ...
        do
        {
          ...
          case 'b':
            ...
            if (maxpdr < minpdr)
            {
              fprintf(stderr, "\nmin seafloor depth exceeds max depth\n");
              exit(1);
            }
  ...
  /* Read in WMO_squares file. */
  fprintf(stderr, "\nReading wmo_squares file.... ");
  /* Read the first line containing the number of squares. */
  fscanf(wmo_squares_file, "%[^\n]", wmo_square);
  /* move past end of line marker */
  getc(wmo_squares_file); 
  /* Convert shipindex to integer */
  num_wmo_squares = atoi(wmo_square);

  /* Initialize wmo_codes table */
  wmo_squares = (int *)calloc(num_wmo_squares, sizeof(int));
  if (wmo_squares == NULL)
  {
    fprintf(stderr, "Unable to allocate memory for wmo_squares table\n");
    exit(1);
  }

  /* Read each line to buf */
  i = 0;
  while ((error = fscanf(wmo_squares_file, "%[^\n]", wmo_square)) == 1)
  {
    /* move past end of line marker */
    getc(wmo_squares_file); 
    /* Test that we are within acceptable range of the table */
    if (i >= 0 && i < wmo_squares_file)
    {
      wmo_squares[i] = atoi(wmo_square);
      i++;
    } /* End of test for index range */
  } /* End of looping over lines in file */
...
/* Initialize WOD dynamic arrays */
  ...
  /* LOAD PROFILES FROM INPUT FILE(S) */

  /* Enter standard level depths in case we have standard level data.  This is only
   * needed for WOD09 and earlier.  WOD13 files read standard level depths from
   * the data by default.  There is a potential bug here in case a single WOD file has
   * mixed standard and obs depth levels (or the multiple input files are mixed).*/
  for (j = 0; j < 40; j++)
    *(depth + j) = *(sdepth + j);
  ...
  fprintf(stderr, "\n  %d accepted stations contained bad scans \n\n", stabad);
  exit(0);
...
void print_usage(char *program)
{
  ...
  fprintf(stderr, "\nConverts .OSD, .CTD, and .PFL files (observed levels)from WOD18 to HydroBase\n");
  fprintf(stderr, "\nUsage: %s filelist -S<shipfile> -W<wmo_squares_file> -T<instr_type>", program);
  fprintf(stderr, "\n   [-B<badfile>] [-D<dirname>] [-E<extent>] [-h] [-O<outfile>] [-R/[b/min_depth/max_depth]/[w]");
  fprintf(stderr, "\n\n  List of filenames must be first argument!");
  fprintf(stderr, "\n  Otherwise, input is expected from stdin.");
  fprintf(stderr, "\n-S : path/filename for OCL/NODC shipcode table in CSV format <OCL code>,<NODC index>.");
  ...
  fprintf(stderr, "\n-R : specifies types of station to reject");
  fprintf(stderr, "\n        b = if bathymetry depth falls outside min/max ex:  -Rb/1/11000");
  fprintf(stderr, "\n        w = if WMO square falls outside range of sigma list files ex:  -Rw");
   fprintf(stderr,"\n        combination of above:  -Rb/1/11000/w");
...
int check_sta()
{
  ...
  /* Reject the entire station if essential values are missing from the header. 
   * Not only are the values used to indicate missing data invalid but their
   * length will corrupt the station file if output. */
  int missing = -9999;
  if (ostation == missing || icruise == missing || year == missing || month == missing || day == missing ||
      (latitude == -9999 && hrightfig[1] == 0) || 
      (longitude == -99999 &&  hrightfig[2] == 0) || 
      levels == missing || nparm == missing)
  {
    bad_hdr.nobs = 0;
    return (0);
  }
  ...
  if (w_flag) {
    /* Reject stations not in oceanic ms10 squares. They will break us
     * when we try hb_bin3d on them. */
    found = 0;
    i = 0;
    while (!found && i < num_wmo_squares)
    {
      found = (wmo_squares[i] == hdr.ms10);
      i++;
    }
    if (!found) 
    {
  ...
  /* HB3 version has slightly different country assignment. The country
   * assignment is done again based on the ship codes table, so this value
   * only persists if the ship table cannot be looked up properly.  Use
   * this as a double check for ship table parsing. */
  ...
  *(origs + origsfig) = '\0';
  ...
  {
    --i;
    int pdr = secval[i] / tenp[srightfig[i]];
    if (pdr > 0 && pdr < 11000)
    {
      /* Ignore nonsensical values */
      hdr.pdr = bad_hdr.pdr = pdr;
    }
  }
  if (b_flag && (hdr.pdr < minpdr || hdr.pdr > maxpdr))
  {
    /* Exclude stations outside specified range */
    bad_hdr.nobs = 0;
    return (0);
  }
   ...
   * HB2 does not have ITS90 capability, so instead of writing to TE or T90, we must convert
   * any explicitly noted T90 to TE and always output temperature data in TE -> ITS-68.*/
  found = 0;
  ii = 0;
   ...
   * Variable codes are determined from WOD tutorial, Table 3, page 17. */

  /* Also, perform a check on the profile flags. check_profile_flag will return 1 for any
   * standard WOD05 flag - so it looks like all profiles will be included.  The real data
   * checking is happening at each level with the data flags. */

  /* TE, SA, OX, ... etc. are declared as an enum property in hydorbase.h */
  ...
  for (j = 0; j < levels; ++j)
  {
    ...
    /* Copy flag for depth.  Note T/F flip from
     * conditional operator. */
    data_flagged[(int)DE][j] = check_depth_flag(zerr[j]) ? 0 : 1;
  ...
  if (hdr.nobs > 30)
  {
    ...
    if (prsint < 10)
    {
      ...
      if (prsint < 1)
      {
        /* Protect against very small values of prsint, particularly 0, which can occur if there is no depth change. */
        prsint = 1;
      }
      delta = (int)(10 / prsint);
...
/****************************************************************************
 * The function check_depth_flag will return 1 for integer inputs listed
 * in the case n: lines and returns zero for any other integer input.
 *
 * Flag = 0 = accepted value
 * Flag = 1 = duplicates or inversions in recorded depth ( same or less than previous depth )
 * Flag = 2 = density inversion
 *
 * Ultimately, the flags listed in this routine are "flags to ignore."
 *
 ***************************************************************************/
int check_depth_flag(int flag)
{
...
int oclread()
{
  ...
  for (i = 0; i < nparm; i++)
  {
    ...
    if ((iend = extracti(0, &totfig, &sigfig, &rightfig, &npinf, missing)) == -1)
      return iend;
    npinfe += npinf;
  ...
  {
    totfig = 1;
    if ((iend = extracti(2, &totfig, &sigfig, &rightfig, &ninfc, missing)) == -1)
      return iend;
    ...
    for (i = 0; i < ninfc; i++)
    {
      ...
      totfig = 1;
      if ((iend = extracti(2, &totfig, &sigfig, &rightfig, &ntypec, missing)) == -1)
        return iend;
      ...
      if (ntypec == 1)
      {
        ...
        if ((iend = extracti(2, &totfig, &sigfig, &rightfig, &origcfig,
                             missing)) == -1)
          return iend;
        if ((iend = extractc(0, &origcfig, origc)) == -1)
          return iend;
      ...
      else if (ntypec == 2)
      {
        ...
        if ((iend = extracti(2, &totfig, &sigfig, &rightfig, &origsfig,
                             missing)) == -1)
          return iend;
        if ((iend = extractc(0, &origsfig, origs)) == -1)
          return iend;
      ...
      else if (ntypec == 3)
      {
        ...
        totfig = 2;
        if ((iend = extracti(2, &totfig, &sigfig, &rightfig, &npi, missing)) == -1)
          return iend;
      ...
      for (j = 0; j < ntsets; j++)
      {
        ...
        for (i = 0; i < *(ntloc + j); i++)
        {
          ...
          if ((iend = extracti(1, (nttotfig + ntoff + i), (ntsigfig + ntoff + i),
                               (ntrightfig + ntoff + i), (ntval + ntoff + i), missing)) == -1)
            return iend;
          totfig = 1;
  ...
  for (j = 0; j < levels; j++)
  {
    ...
    /* For observed records or all records in WOD13+, depth is stored in the file. */
    if (isoor == 0 || wodform == 'C')
    {

      if ((iend = extracti(1, (ztotfig + j), (zsigfig + j),
                           (zrightfig + j), (depth + j), missing)) == -1)
        return iend;
      totfig = 1;
    ...
    for (i = 0; i < nparm; i++)
    {
      ...
      if ((iend = extracti(1, (dtotfig + doff + j), (dsigfig + doff + j),
                           (drightfig + doff + j), (dataval + doff + j), missing)) == -1)
        return iend;
...
void spacer(int intime)
{
  ...
  if (intime == 1)
  {
    ...
    ntsets = 1;
    ntsetsmax = 1;
    if ((ntcode = calloc(ntsets * maxtax, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((ntloc = calloc(ntsets * maxtax, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((nttotfig = calloc(ntsets * maxtax, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((ntsigfig = calloc(ntsets * maxtax, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((ntrightfig = calloc(ntsets * maxtax, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((ntval = calloc(ntsets * maxtax, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((nterr = calloc(ntsets * maxtax, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((ntoerr = calloc(ntsets * maxtax, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);

    /* ALLOCATE SPACE FOR DEPTH AND MEASURED PARAMETERS
     * WOD09 had 40 for all sizes in this block.
     * WOD13 & 18 has 137 standard depth levels. */
    zsize = 137;
    zsizemax = 137;
    if ((ztotfig = calloc(zsize, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR DEPTH DATA\n");
    if ((zsigfig = calloc(zsize, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR DEPTH DATA\n");
    if ((zrightfig = calloc(zsize, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR DEPTH DATA\n");
    if ((depth = calloc(zsize, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR DEPTH DATA\n");
    if ((zerr = calloc(zsize, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR DEPTH DATA\n");
    if ((zoerr = calloc(zsize, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR DEPTH DATA\n");
    ...
    isize = 137;
    isizemax = 137;
    if ((dtotfig = calloc(isize, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR MEASURED DATA\n");
    if ((dsigfig = calloc(isize, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR MEASURED DATA\n");
    if ((drightfig = calloc(isize, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR MEASURED DATA\n");
    if ((dataval = calloc(isize, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR MEASURED DATA\n");
    if ((derr = calloc(isize, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR MEASURED DATA\n");
    if ((doerr = calloc(isize, sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR MEASURED DATA\n");
  ...
  else if (intime == 2)
  {
    ...
    ntsetsmax = ntsets;
    if ((ntcode = realloc(ntcode, ntsets * maxtax * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((ntloc = realloc(ntloc, ntsets * maxtax * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((nttotfig = realloc(nttotfig, ntsets * maxtax * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((ntsigfig = realloc(ntsigfig, ntsets * maxtax * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((ntrightfig = realloc(ntrightfig, ntsets * maxtax * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((ntval = realloc(ntval, ntsets * maxtax * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((nterr = realloc(nterr, ntsets * maxtax * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
    if ((ntoerr = realloc(ntoerr, ntsets * maxtax * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR %d TAXA SETS\n", ntsets);
  ...
  else if (intime == 3)
  {
    ...
    if ((ztotfig = realloc(ztotfig, zsize * sizeof(int))) == NULL)
      printf(" #1 NOT ENOUGH SPACE IN MEMORY FOR DEPTH DATA\n");
    if ((zsigfig = realloc(zsigfig, zsize * sizeof(int))) == NULL)
      printf(" #2 NOT ENOUGH SPACE IN MEMORY FOR DEPTH DATA\n");
    if ((zrightfig = realloc(zrightfig, zsize * sizeof(int))) == NULL)
      printf(" #3 NOT ENOUGH SPACE IN MEMORY FOR DEPTH DATA\n");
    if ((depth = realloc(depth, zsize * sizeof(int))) == NULL)
      printf(" #4 NOT ENOUGH SPACE IN MEMORY FOR DEPTH DATA\n");
    if ((zerr = realloc(zerr, zsize * sizeof(int))) == NULL)
      printf(" #5 NOT ENOUGH SPACE IN MEMORY FOR DEPTH DATA\n");
    if ((zoerr = realloc(zoerr, zsize * sizeof(int))) == NULL)
  ...
  else if (intime == 4)
  {
    ...
    if ((dtotfig = realloc(dtotfig, isize * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR MEASURED DATA");
    if ((dsigfig = realloc(dsigfig, isize * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR MEASURED DATA");
    if ((drightfig = realloc(drightfig, isize * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR MEASURED DATA");
    if ((dataval = realloc(dataval, isize * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR MEASURED DATA");
    if ((derr = realloc(derr, isize * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR MEASURED DATA");
    if ((doerr = realloc(derr, isize * sizeof(int))) == NULL)
      printf(" NOT ENOUGH SPACE IN MEMORY FOR MEASURED DATA");
 ...
 * Otherwise, zero is returned for sucessful extraction.
 *
 **********************************************************/
